...
import db.mmethods.nonlinear.methods.*
...
fun main(args: Array<String>) {
    val fn = DifferentiableTwiceFunction(
        ...
        Function({ x -> 3.0 * pow(x, 2.0) - 2.0 * x - 1 }),
        Function({ x -> 6.0 * x - 2.0 }, SimpleRangeToBoolean(3.0), EmptyRangeToBoolean())
    )
...
private fun Validated<Errors, IndexedValue<Double>>.processResult() = fold(
    ...
    { (iterations, root) -> println("Після $iterations ітерацій отримано корінь $root") }
)
